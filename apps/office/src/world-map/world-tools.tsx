...
import { upgradeModuleWorld, compactModuleWorld } from './module-upgrade.js';
...
export function WorldTools({
  ...
  return (
    <div className="world-editor-hud">
      ...
      >
        {(world.map.version !== 1
          ? (['select', 'office', 'move'] as const)
          : (['select', 'move'] as const)
        )
          .filter((value) => value !== 'move' || !object)
          .map((value) => (
            <button
              key={value}
        ...
        {(['walls', 'furniture'] as const).map((value) => (
          <button
            key={value}
            disabled={editor.busy}
      ...
      <aside
        hidden={tool === 'office' || creatingMeeting}
                    ...
                    <label>
                      ...
                      >
                        {!rooms.some((room) => room.id === meetingRoomId) && (
                          <option value={meetingRoomId}>Unavailable room · {meetingRoomId}</option>
                        )}
                        {rooms.map((room) => (
                  ...
                  {area.id === world.map.primaryLobbyId && (
                    <label>
                      ...
                      >
                        ...
                        {map.areas
                          .filter((other) => other.id !== area.id && other.binding.type === 'lobby')
                          .map((other) => (
