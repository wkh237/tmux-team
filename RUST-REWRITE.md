...
| Request transport and exact finals    | Shared `request-context`, `exchange-retention`, `response-integrity`, `durable-talk`, `transport-safety` and `talk-completion` E2E retain original prompts, UUID attribution, late finals, virtualized terminal output, summary ordering, no replay and waiter cleanup. Completion comes from SQLite, not terminal boundaries.                        |
