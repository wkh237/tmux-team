...
fn registration_retirement_reopen_and_delivery_preserve_terminal_state() {
    ...
    let mut storage = Storage::open(&path).unwrap();
    assert_eq!(storage.health().unwrap().schema_version, 15);
    assert_eq!(
...
fn schema_nine_upgrade_is_atomic_and_preserves_identity() {
    ...
    storage.connection().unwrap().execute_batch("DROP TABLE identity_hooks; DROP TABLE office_local_profiles; DROP TABLE office_local_blocks; DROP TABLE office_local_worlds; DROP TABLE request_recipient_attention_identities; DROP TABLE identity_metadata; DROP TABLE office_board_operations; DROP TABLE office_board_entries; DROP TABLE office_board_state; DELETE FROM _migrations WHERE version IN (10, 11, 12, 13, 14, 15);
