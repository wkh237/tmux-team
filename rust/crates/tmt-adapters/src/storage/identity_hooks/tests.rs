...
fn schema_nine_upgrade_is_atomic_and_preserves_identity() {
    ...
    storage.connection().unwrap().execute_batch("DROP TABLE identity_hooks; DROP TABLE office_local_blocks; DROP TABLE office_local_worlds; DROP TABLE request_recipient_attention_identities; DROP TABLE identity_metadata; DELETE FROM _migrations WHERE version IN (10, 11, 12, 13);
